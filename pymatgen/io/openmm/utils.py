...
def molgraph_from_atoms_bonds(
    atoms: Iterator[openff.toolkit.topology.Atom],
    bonds: Iterator[openff.toolkit.topology.Bond],
    ...
    # formal_charge = sum(atom.formal_charge for atom in atoms_list)
    ...
    molgraph = MoleculeGraph.with_empty_graph(
        molecule=mol, name=name, edge_weight_name="weight", edge_weight_units=""
    )
...
def openff_mol_to_molgraph(openff_mol: tk.topology.Molecule) -> MoleculeGraph:
    """
